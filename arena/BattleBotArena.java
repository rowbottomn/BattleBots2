...
//ROWBOTTOM 070926 //ROWBOTTOM 082023
 ...
 * @version <br>1.7 (Aug 9, 2011) - Converted to an application that can be JAR'ed -- the mouse wheel was not working well when embedded in a web page
 * @version <br>1.8 (Nov 30, 2011) - Fixed audio bug *
 * @author Sam Scott
 ...
 * @version <br>2.1 (Nov 16, 2015) - Rowbottom extends limited ammo functionality so that liveBots can pickup used ammo off deadBots. 
 * @version <br>2.2 (Nov 11, 2017) - Rowbottom carries fixes and tweaks made to BattleBots 1 to BattleBots 2.
 * @version <br>2.3 (Apr 23, 2019) -//Rowbottom Adding Ammo Display for corpses
 ...
 * @version <br>2.5 (Dec 20 2024) -//Rowbottom Added moving, sensing, dodging, attacking interfaces to make more scaffolding for students.
 * @version <br>2.6 (Jul 09 2026) -//Rowbottom Finally getting rid of deprecated Applet and AudioClip classes.  Replaced with JavaFX MediaPlayer and Media classes.
 * 
...
public class BattleBotArena extends JPanel implements MouseListener, MouseWheelListener, MouseMotionListener, ActionListener, Runnable {
     ...
     */
    public static final int RIGHT_EDGE = 1400; //arena panel is 
    /**
     * Bottom edge of the screen
     */
    public static final int BOTTOM_EDGE = 700; // arena panel height is this constant + TEXT_BUFFER
    /**
     ...
     * survival points 
     */
    public static final double  POINTS_PER_SECOND = 0.08; //ROWBOTTOM 082023 increased to 0.08 reward dodging ROWBOTTOM changed from 0.1 to discourage camping
    /**
     ...
     */
    public static final double     BOT_SPEED = Math.floor((RIGHT_EDGE-LEFT_EDGE)/240);//ROWBOTTTOM changed from 3 to accomodate larger screensROWBOTTOM changed from 2.5 to fix game balance and bots getting hung up on tombstones
    /**
     * Bullet speed in pixels/frame
     */
    public static final double     BULLET_SPEED = Math.floor((RIGHT_EDGE-LEFT_EDGE)/120);//ROWBOTTTOM changed from 6 to accomodate larger screens
    /**
     ...
     * Dead Bot image
     */
    private final Image deadBot;
    /**
     * Overheated Bot image
     */
    private final Image overheated;
    /**
     ...
     * ROWBOTTOM 070926 - this is not used currently but should be used to fix the cpu times for each bot after unpausing the game.  Currently the cpu times are not correct after a pause.
     */
     ...
     */
    private final Image[] replayImages = new Image[NUM_FRAMES];
    /**
     ...
     * loaded.
     */
    private LinkedList<Image> imagesToLoad = new LinkedList<>();
    /**
     * Holds an audioclip for arena sound.
     */
    Clip death, fanfare, shot, drone, open, stop, gameOver, overheat;
    ...
    private void fullReset()
    {
        ...
        if (soundOn) open.start();    // Play the fanfare
    ...
    private void reset()
    {
        ...
        messages = new LinkedList<>();// clear the messag buffer
      ...
      //offets made to move
        ...
        if (state == TEST_MODE)
        {
            ...
            sendMessage(-1,"    ");
            sendMessage(-1,"Ammo is limited.  Bots start with a limited amount of ammo and ");
            sendMessage(-1,"cannot fire if they have no ammo left.  ");
            sendMessage(-1,"Live bots can pick up ammo from dead bots but drawing near to them.");
            sendMessage(-1,"    ");
    ...
    public void actionPerformed(ActionEvent ace)
    {
        ...
        if (state == GAME_ON && countDown <= 0 || state == TEST_MODE && ready)
        {
            ...
            else //**** GAME IS ON
            {
                ...
                BotInfo[] deadBots = new BotInfo[NUM_BOTS-(round-1)*ELIMINATIONS_PER_ROUND-botsLeft];//the size is a bit of a hangover from when round eliminated bots
        ...
        else // countdown to the start
        {
            ...
            if (countDown == 0)
            {
                startTime = System.currentTimeMillis();
                if (soundOn)
                    drone.loop(Clip.LOOP_CONTINUOUSLY);
            }
            // paint the screen
            paintBuffer();
        }
    ...
    private void paintBuffer()
    {
        ...
        if (ready) // avoid race condition on startup
        {
            ...
            {
                LinkedList<Image> newImagesToLoad = new LinkedList<>();
